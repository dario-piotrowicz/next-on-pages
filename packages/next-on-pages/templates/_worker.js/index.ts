...
declare const __ALSes_PROMISE__: Promise<null | {
	...
	cloudflareAsyncLocalStorage: AsyncLocalStorage<unknown>,
 }>;
...
export default {
	async fetch(request, env, ctx) {
		...
		const { envAsyncLocalStorage, cloudflareAsyncLocalStorage } = asyncLocalStorages;
		...
		return envAsyncLocalStorage.run(
			...
			{ NODE_ENV: __NODE_ENV__, SUSPENSE_CACHE_URL },
			async () => {
				return cloudflareAsyncLocalStorage.run({ env, ctx, cf: request.cf }, async () => {
